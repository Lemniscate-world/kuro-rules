...
| **Neural-Agent** |  60%  | Actif | Agent auto-correcteur pour entraînement RN. 87 tests, pipeline CPU validé (tiny-gpt2 + LoRA, 5 steps). Règle MHA wired. Modèle pas encore entraîné (Kaggle notebook prêt). Boucle fermée : NeuralDBG → Diagnostic → Fix. |
| **Aladin** | 40% | Actif | Architecture Transformer & recherche LLM. |
| **Astral** |  20%  | Actif | Multi-repo intelligence avec client Hindsight. Data Gravity moat implémenté, Next.js MVP. |
...
| **Aquarium** |  15%  | Prototypage | IDE visuel (Tauri) pour NeuralDBG. MVP livré, dormant. Export JSON + 14 tests Aquarium. |
...
| **TokenWise** |  5%  | Prototypage | Analyse et réduction du coût par tokens utilisés. Phase validation Mom Test. |
...
| **Automatons** |  5%  | Prototypage | Agent orchestration and automation framework. |
...
| Projet | Progression | Statut | Description |
|--------|-------------|--------|-------------|
| **EchoX** |  27%  | Actif | SRS avec forgetting curves (PyQt6). Validation L1 complète ($23.9B marché), GUI desktop opérationnelle. 6 tests. Structure Tauri initialisée. |
...
| Projet | Progression | Statut | Description |
|--------|-------------|--------|-------------|
| **Dissect** |  35%  | Actif | Outil d'audit et de visualisation de l'orchestration des agents IA. 9 tests, branch: main. |
