...
$PROJECT_NAME = Split-Path $TargetDir -Leaf
...
function Expand-TemplateContent {
    param([string]$Content, [hashtable]$Context)
    $result = $Content
...
function Install-RuleFile {
    param([string]$SourcePath, [string]$DestinationPath, [switch]$Overwrite)
    ...
    if ($UseSymlinks) {
        ...
        Write-Host "LINK  $DestinationPath"
        return
    }
    ...
    if ((Test-Path -LiteralPath $DestinationPath) -and -not $Overwrite) {
        try {
            ...
            # Broken or unreadable existing file: fall through and replace it.
        }
    }
...
function Install-ScaffoldFile {
    param([string]$SourcePath, [string]$DestinationPath, [bool]$IsTemplate, [hashtable]$Context, [switch]$Overwrite)
    Ensure-ParentDirectory -Path $DestinationPath
    if ((Test-Path -LiteralPath $DestinationPath) -and -not $Overwrite) {
        Write-Host "SKIP  $DestinationPath"
        return
    ...
    Write-Host "MAKE  $DestinationPath"
}

function Remove-LegacyRootCopilotCopy {
    $rootCopy = Join-Path $TargetDir "copilot-instructions.md"
    $githubCopy = Join-Path $TargetDir ".github/copilot-instructions.md"

    if ((Test-Path -LiteralPath $rootCopy) -and (Test-Path -LiteralPath $githubCopy)) {
        try {
            ...
            if ($rootHash -eq $githubHash) {
                Remove-Item -LiteralPath $rootCopy -Force
                Write-Host "CLEAN $rootCopy"
            } else {
                Write-Host "WARN  Legacy root copilot-instructions.md differs from canonical .github copy; manual review required" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "WARN  Could not compare legacy root copilot-instructions.md; manual review required" -ForegroundColor Yellow
        }
    }
}
function Ensure-PrivateIgnoreBlock {
    ...
    Write-Host "EDIT  $gitignorePath"
}

$context = Get-ProjectDefaults -Name $PROJECT_NAME

Write-Host "Installing kuro-rules into $TargetDir" -ForegroundColor Cyan

foreach ($file in $RULE_FILES) {
    $src = Join-Path $RULES_DIR $file.Source
    $dst = Join-Path $TargetDir $file.Target
    Install-RuleFile -SourcePath $src -DestinationPath $dst -Overwrite:$ForceRules
}

$preCommitSrc = Join-Path $RULES_DIR ".pre-commit-config.yaml"
$preCommitDst = Join-Path $TargetDir ".pre-commit-config.yaml"
if (-not (Test-Path $preCommitDst)) {
    Copy-Item -Path $preCommitSrc -Destination $preCommitDst -Force
    Write-Host "COPY  $preCommitDst"
}

foreach ($file in $SCAFFOLD_FILES) {
    $src = Join-Path $RULES_DIR $file.Source
    $dst = Join-Path $TargetDir $file.Target
    Install-ScaffoldFile -SourcePath $src -DestinationPath $dst -IsTemplate $file.Template -Context $context -Overwrite:$ForceScaffold
}

Ensure-PrivateIgnoreBlock
Remove-LegacyRootCopilotCopy

Write-Host ""
Write-Host "Done. Rules installed, prompts live in prompts/, and staged validation lives in research/." -ForegroundColor Green
